...
author: "JD Reigrut, Juan Carlos Villaseñor-Derbez"
date: "2025-12-09"
format:
  html:
    toc: true
...
## Removing column of duplicate site names 
```{r}
sites_key_geo <- subset(sites_key_geo, select = -Localidad_geo)
```

## Evaluating how many columns do not have associated coordinates
```{r}
sites_key_geo %>% 
  summarise(
    na_geom = sum(is.na(geom)),         
    empty_geom = sum(st_is_empty(geom)) 
  )
#No columns with NAs or empty values for associated coordinates
#1,540 locations with geographic coordinate values
```

# Step 6) Visualize data before exporting
...
# Step 7) Saving final clean geopackage
